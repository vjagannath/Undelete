 ...
 */
  ...
  if ( rip == NULL && err_code == ENOENT) {
	...
	if ( (rip = alloc_inode((ldirp)->i_dev, bits, uid, gid)) == NULL) {
		...
		if(undelete_cache_item_count > 0) {
			
			int i=1;
			
			Undelete_Inode_Cache[0]->i_nlinks--;
			IN_MARKDIRTY(Undelete_Inode_Cache[0]);
			put_inode(Undelete_Inode_Cache[0]);
			
			while(i <= undelete_cache_item_count) {
				Undelete_Inode_Cache[i-1] = Undelete_Inode_Cache[i];
				i++;
			}
			undelete_cache_item_count--;
