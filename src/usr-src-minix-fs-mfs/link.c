...
struct inode* Undelete_Inode_Cache[MAX_UNDELETE_CACHE] = {NULL};
int undelete_cache_item_count = 0;
...
/* Unlink 'file_name'; rip must be the inode of 'file_name' or NULL. */
  ...
  if (r == OK) {
	...
	if(rip->i_nlinks == 0) {
		...
		if(undelete_cache_item_count < MAX_UNDELETE_CACHE) {
			
			if(Undelete_Inode_Cache[undelete_cache_item_count]!= NULL){
				Undelete_Inode_Cache[undelete_cache_item_count]->i_nlinks--;
				IN_MARKDIRTY(Undelete_Inode_Cache[undelete_cache_item_count]);
				put_inode(Undelete_Inode_Cache[undelete_cache_item_count]);
			}
		  
			Undelete_Inode_Cache[undelete_cache_item_count++] = rip;
		}
		else {
			undelete_cache_item_count = 0;
			Undelete_Inode_Cache[undelete_cache_item_count]->i_nlinks--;
			IN_MARKDIRTY(Undelete_Inode_Cache[undelete_cache_item_count]);
			put_inode(Undelete_Inode_Cache[undelete_cache_item_count]);
